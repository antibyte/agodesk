...
test("buildChatAttachmentPrepareMessage setzt Metadaten und prepare_id", { concurrency: false }, () => {
...
test("handleChatAttachmentPreparedMessage loest wartenden prepare auf", { concurrency: false }, async () => {
  const prepareId = "prep-123";
  const sent: WsMessage[] = [];
  const waitPromise = prepareChatAttachment(async (message) => {
    sent.push(message);
  }, {
    sessionId: "sess-a",
...
test("rejectPendingAttachmentPrepare und rejectAttachmentPrepareByRequestId", { concurrency: false }, async () => {
  const sent: WsMessage[] = [];
  const waitPromise = prepareChatAttachment(async (message) => {
    sent.push(message);
  }, {
  ...
  const secondWait = prepareChatAttachment(async (message) => {
